...
class HybridFaceAntiSpoofPlugin : HybridFaceAntiSpoofPluginSpec() {
        ...
        processingExecutor.submit {
            try {
                ...
                val resultMap = FaceAntiSpoofResult(
                    ...
                    isLive = result.combinedScore > 0.5f && result.laplacianScore > FaceAntiSpoofingAdvanced.LAPLACIAN_THRESHOLD,
                    label = if (result.combinedScore > 0.5f && result.laplacianScore > FaceAntiSpoofingAdvanced.LAPLACIAN_THRESHOLD) "Live Face" else "Spoof Face",
                    error = null
                )
    ...
    private fun extractNV21(image: ImageProxy): Triple<ByteArray, Int, Int>? {
